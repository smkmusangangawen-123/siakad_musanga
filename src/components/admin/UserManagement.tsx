...
import { User, UserRole, Kelas, Jurusan, AbsensiRecord, MataPelajaran } from '../../types';
import { exportToCSV } from '../../utils/csvHelper';
import { AccountPasswordManager } from './AccountPasswordManager';
import { ManajemenMapel } from './ManajemenMapel';
...
interface UserManagementProps {
  ...
  onAddSubject?: (newSubject: MataPelajaran) => void;
  onUpdateSubject?: (updatedSubject: MataPelajaran) => void;
  onDeleteSubject?: (subjectId: string) => void;
  onOpenSchoolSettings?: () => void;
}

type MasterTab = 'semua_akun' | 'siswa' | 'guru' | 'karyawan' | 'kelas' | 'jurusan' | 'mapel';
    ...
    if (modalType === 'add_siswa') {
      ...
      if (existing) {
        onUpdateUser({
          ...
          name: formName.trim(),
          email: formEmail.trim(),
          avatar: formAvatar.trim() || existing?.avatar || PRESET_AVATARS.siswaPria[0].url,
    ...
    if (modalType === 'add_guru') {
      ...
      if (existing) {
        onUpdateUser({
          ...
          name: formName.trim(),
          email: formEmail.trim(),
          avatar: formAvatar.trim() || existing?.avatar || PRESET_AVATARS.guruPria[0].url,
          username: formUsername.trim() || existing.username || formEmail.split('@')[0],
    ...
    if (modalType === 'add_karyawan') {
      ...
      if (existing) {
        onUpdateUser({
          ...
          name: formName.trim(),
          email: formEmail.trim(),
          avatar: formAvatar.trim() || existing?.avatar || PRESET_AVATARS.guruPria[1].url,
          username: formUsername.trim() || existing.username || formEmail.split('@')[0],
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-2xl border border-slate-200 p-6 shadow-xs">
        ...
        <div className="flex flex-wrap gap-2 border-b border-slate-100 mt-6 pt-2 pb-0">
          ...
          >
            <Award className="w-4 h-4" /> Mata Pelajaran ({subjects.length})
          </button>
        </div>
      </div>
      ...
      {activeTab !== 'semua_akun' && activeTab !== 'mapel' && (
        <>
                    ...
                    <tr key={t.id} className="hover:bg-slate-50/80 transition-colors">
                      <td className="py-3 px-4">
                        <div className="flex items-center gap-2.5">
                          ...
                          >
                            <img
                              src={t.avatar || 'https://images.unsplash.com/photo-1507003211169?w=150&auto=format&fit=crop&q=80'}
                              alt={t.name}
                              className="w-9 h-9 rounded-full object-cover border border-slate-200 group-hover:ring-2 group-hover:ring-blue-500 transition-all shadow-xs"
                              referrerPolicy="no-referrer"
                            />
                    ...
                    <tr key={k.id} className="hover:bg-slate-50/80 transition-colors">
                      <td className="py-3 px-4">
                        <div className="flex items-center gap-2.5">
                          ...
                          >
                            <img
                              src={k.avatar || 'https://images.unsplash.com/photo-1472099645785?w=150&auto=format&fit=crop&q=80'}
                              alt={k.name}
                              className="w-9 h-9 rounded-full object-cover border border-slate-200 group-hover:ring-2 group-hover:ring-blue-500 transition-all shadow-xs"
                              referrerPolicy="no-referrer"
                            />
