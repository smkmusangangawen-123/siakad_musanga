...
export const SchoolSettingsModal: React.FC<SchoolSettingsModalProps> = ({
  ...
  const handleLogoFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file.size > 2 * 1024 * 1024) {
      setUploadError('Ukuran file logo terlalu besar (Maksimal 2MB).');
      return;
    }
    ...
    reader.onload = () => {
      ...
      setFormData((prev) => ({ ...prev, logoUrl: result, titleLogoUrl: result }));
    };
    reader.readAsDataURL(file);
  };

  // Background Wallpaper File Upload (Base64)
  const handleBgFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file.size > 4 * 1024 * 1024) {
      setBgUploadError('Ukuran file foto wallpaper terlalu besar (Maksimal 4MB).');
      return;
    }
