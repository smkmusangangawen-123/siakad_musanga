...
export const AbsensiOnline: React.FC<AbsensiOnlineProps> = ({
  ...
  const [selectedTeacherId, setSelectedTeacherId] = useState<string>(isAdmin ? 'semua' : currentUser.id);
  ...
  const [rekapViewMode, setRekapViewMode] = useState<'ringkasan-guru' | 'detail-log'>(isAdmin ? 'ringkasan-guru' : 'detail-log');
  ...
  const filteredRekapRecords = useMemo(() => {
    return staffAttendanceList.filter((rec) => {
      // User role isolation: If not admin, only show currentUser's records!
      if (!isAdmin) {
        const isSelf =
          rec.pegawaiId === currentUser.id ||
          rec.nipOrNik === currentUser.nip ||
          rec.pegawaiNama.toLowerCase() === currentUser.name.toLowerCase();
      ...
      // 2. Filter Teacher / Staff ID (for admin)
      if (isAdmin && selectedTeacherId !== 'semua' && rec.pegawaiId !== selectedTeacherId) {
        return false;
      }
  ...
  const perTeacherSummaries = useMemo(() => {
    ...
    // List of teachers to summarize: If not admin, only include currentUser!
    const targetTeachers = !isAdmin
      ? teacherAndStaffList.filter(
          (t) =>
            t.id === currentUser.id ||
            t.nip === currentUser.nip ||
            t.nama.toLowerCase() === currentUser.name.toLowerCase()
        )
      : teacherAndStaffList;
  ...
  }, [teacherAndStaffList, filteredRekapRecords, rekapKategoriFilter, isAdmin, currentUser]);
  ...
  const filteredStudents = attendanceList.filter((a) => {
    // If student, only show logged-in student's records
    if (isSiswa) {
      const isSelf =
        a.siswaId === currentUser.id ||
        (currentUser.nisn && a.nisn === currentUser.nisn) ||
        a.siswaNama.toLowerCase() === currentUser.name.toLowerCase();
      if (!isSelf) return false;
    } else if (isOrangTua) {
      const isChild =
        (currentUser.childId && a.siswaId === currentUser.childId) ||
        (currentUser.nisn && a.nisn === currentUser.nisn) ||
        a.siswaNama.toLowerCase() === currentUser.name.toLowerCase();
  ...
  const studentUsers = useMemo(() => {
    if (isSiswa) {
      const self = allUsers.find(
        (u) =>
          u.id === currentUser.id ||
          (currentUser.nisn && u.nisn === currentUser.nisn) ||
          u.name.toLowerCase() === currentUser.name.toLowerCase()
      );
      return self ? [self] : [currentUser];
    }
    if (isOrangTua) {
      const child = allUsers.find(
        (u) =>
          (currentUser.childId && u.id === currentUser.childId) ||
          (currentUser.nisn && u.nisn === currentUser.nisn) ||
          u.name.toLowerCase() === currentUser.name.toLowerCase()
      );
      return child ? [child] : [];
  ...
  }, [allUsers, isSiswa, isOrangTua, currentUser]);
  ...
  const filteredStaffDaily = staffAttendanceList.filter((a) => {
    // If not admin, only show logged-in user's own records!
    if (!isAdmin) {
      const isSelf =
        a.pegawaiId === currentUser.id ||
        a.nipOrNik === currentUser.nip ||
        a.pegawaiNama.toLowerCase() === currentUser.name.toLowerCase();
      if (!isSelf) return false;
    }
  ...
  // Calculate today staff counts (school-wide for admin)
  ...
  // Personal metrics for logged-in teacher / staff
  const myStaffRecords = staffAttendanceList.filter(
    (r) =>
      r.pegawaiId === currentUser.id ||
      r.nipOrNik === currentUser.nip ||
      r.pegawaiNama.toLowerCase() === currentUser.name.toLowerCase()
  );
  const myTodayRecord = myStaffRecords.find((r) => r.tanggal === todayStr);
  const myTotalHadir = myStaffRecords.filter((r) => r.status === 'Hadir').length;
  const myTotalDinas = myStaffRecords.filter((r) => r.status === 'Dinas Luar').length;
  const myTotalIzin = myStaffRecords.filter((r) => ['Izin', 'Sakit', 'Cuti'].includes(r.status)).length;
        ...
        <div>
          <div className="flex items-center gap-2">
            ...
            <span className="px-2.5 py-0.5 rounded text-[10px] font-bold bg-emerald-100 text-emerald-800 uppercase flex items-center gap-1">
              <Sparkles className="w-3 h-3" /> {isAdmin ? 'Guru, Karyawan & Siswa' : isGuru ? 'Presensi Guru & Staf' : 'Presensi Siswa'}
            </span>
          </div>
          <h2 className="text-xl font-extrabold text-slate-900 mt-1.5 flex items-center gap-2">
            <MapPin className="w-5 h-5 text-blue-600" /> {isAdmin ? 'Presensi Kehadiran Online Terpadu' : `Presensi Kehadiran - ${currentUser.name}`}
          </h2>
        ...
        <div className="flex items-center gap-1 bg-slate-100 p-1.5 rounded-xl border border-slate-200 shrink-0 overflow-x-auto">
          {(isAdmin || isGuru) && (
            ...
            >
              ...
              <span>{isAdmin ? 'Absen Guru & Staf' : 'Presensi Masuk/Pulang'}</span>
            </button>
          )}
          ...
          {(isAdmin || isGuru) && (
            ...
            >
              ...
              <span>{isAdmin ? 'Rekap Guru/Staf' : 'Rekap Kehadiran Saya'}</span>
            </button>
          )}

          {(isAdmin || isGuru || isSiswa || isOrangTua) && (
            <button
            ...
            >
              ...
              <span>{isSiswa || isOrangTua ? 'Rekap Presensi Saya' : 'Rekap Absensi Siswa'}</span>
              <span className="px-1.5 py-0.2 rounded text-[9px] bg-emerald-500 text-white font-bold">SINKRON</span>
            </button>
          )}

          {(isAdmin || isSiswa || isOrangTua) && (
            <button
            ...
            >
              ...
              <span>{isSiswa ? 'Absen Mandiri' : 'Presensi Siswa'}</span>
            </button>
          )}
        </div>
      </div>
            ...
            {isAdmin ? (
              ...
              <>
                <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-xs flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Status Presensi Hari Ini</p>
                    <p className="text-sm font-extrabold text-slate-900 mt-0.5">
                      {myTodayRecord ? (
                        <span className="text-emerald-600">{myTodayRecord.status}</span>
                      ) : (
                        <span className="text-amber-600">Belum Presensi</span>
                      )}
                    </p>
                    <p className="text-[10px] text-slate-500 font-semibold">
                      {myTodayRecord ? `Valid ${myTodayRecord.waktuMasuk || ''}` : 'Silakan Catat Presensi'}
                    </p>
                  </div>
                </div>
                ...
                <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-xs flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Jam Masuk / Pulang</p>
                    <p className="text-sm font-extrabold text-slate-900 mt-0.5 font-mono">
                      {myTodayRecord?.waktuMasuk || '--:--'} / {myTodayRecord?.waktuPulang || '--:--'}
                    </p>
                    <p className="text-[10px] text-purple-600 font-semibold">Waktu Tercatat</p>
                  </div>
                </div>
                ...
                <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-xs flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Total Kehadiran Saya</p>
                    <p className="text-xl font-extrabold text-emerald-600 mt-0.5">{myTotalHadir} Hari Hadir</p>
                    <p className="text-[10px] text-slate-500 font-semibold">Riwayat Terverifikasi</p>
                  </div>
                </div>
                ...
                <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-xs flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Izin / Sakit / Dinas</p>
                    <p className="text-xl font-extrabold text-amber-600 mt-0.5">{myTotalIzin + myTotalDinas} Hari</p>
                    <p className="text-[10px] text-slate-500 font-semibold">Tercatat di Database</p>
                  </div>
                </div>
              </>
            )}
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-3">
              <div>
                <h3 className="font-extrabold text-slate-900 text-base flex items-center gap-2">
                  <FileText className="w-5 h-5 text-blue-600" /> {isAdmin ? 'Log Presensi Harian Guru & Karyawan' : `Log Presensi Pribadi (${currentUser.name})`}
                </h3>
                <p className="text-xs text-slate-500">
                  {isAdmin
                    ? 'Daftar riwayat kehadiran harian seluruh guru, staf administrasi, dan karyawan sekolah.'
                    : 'Daftar riwayat presensi masuk dan pulang Anda yang tercatat secara valid di sistem.'}
                </p>
              </div>
              ...
              <div className="flex flex-wrap items-center gap-2">
                ...
                >
                  ...
                  <span>{isAdmin ? 'Buka Rekap Bulanan/Mingguan' : 'Buka Rekap Kehadiran Saya'}</span>
                </button>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              ...
              <div>
                ...
                {isAdmin ? (
                  ...
                  <div className="px-3 py-2 bg-slate-100 border border-slate-200 rounded-xl text-xs font-bold text-slate-900 truncate">
                    {currentUser.name} (NIP: {currentUser.nip || '-'})
                  </div>
                )}
              </div>
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
              <div>
                <h3 className="font-bold text-slate-900 text-base flex items-center gap-2">
                  ...
                  {isSiswa
                    ? `Riwayat Presensi Mandiri (${currentUser.name})`
                    : isOrangTua
                    ? 'Riwayat Presensi Kehadiran Siswa (Anak)'
                    : 'Riwayat Presensi Kehadiran Siswa'}
                </h3>
                <p className="text-xs text-slate-500">
                  {isSiswa
                    ? 'Daftar riwayat presensi harian mandiri Anda yang telah tercatat dan terverifikasi GPS.'
                    : 'Daftar riwayat kehadiran presensi siswa terverifikasi sistem GPS.'}
                </p>
              </div>
