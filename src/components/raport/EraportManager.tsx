...
import { RaportData, User, NilaiSiswa, SchoolSettings, Kelas, RaportPageSelection, MataPelajaran } from '../../types';
...
import { RaportValidationModal } from './RaportValidationModal';
...
export const EraportManager: React.FC<EraportManagerProps> = ({
  ...
  const handleExecuteDownloadPDF = async (customPages?: RaportPageSelection) => {
  ...
  const handleDownloadBulkPDF = async () => {
    ...
    try {
      ...
      alert('Terjadi kesalahan saat memproses bulk PDF.');
    } finally {
      setIsGeneratingPDF(false);
    }
  };
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-xs flex flex-col lg:flex-row lg:items-center justify-between gap-4">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          {/* QR Code Validation Modal Trigger */}
          <button
            onClick={() => setIsValidationModalOpen(true)}
            className="px-3.5 py-2 bg-emerald-50 hover:bg-emerald-100 text-emerald-900 border border-emerald-300 text-xs font-bold rounded-xl transition-all flex items-center gap-1.5 cursor-pointer shadow-xs"
            title="Lihat & Uji Validasi QR Code Keaslian Dokumen e-Raport"
          >
            <QrCode className="w-4 h-4 text-emerald-700" />
            <span>QR Validasi Dokumen</span>
          </button>
          ...
          <button
            ...
            className="px-4 py-2 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-500 hover:to-indigo-500 text-white text-xs font-black rounded-xl shadow-md transition-all flex items-center gap-2 cursor-pointer disabled:opacity-50"
          >
            {isGeneratingPDF ? (
          ...
          <button
            ...
            disabled={isGeneratingPDF}
            className="px-3.5 py-2 bg-slate-900 hover:bg-slate-800 text-white text-xs font-bold rounded-xl shadow-md transition-all flex items-center gap-2 cursor-pointer disabled:opacity-50"
          >
            {isGeneratingPDF ? (
      ...
      {previewTab === 'grades' && (
        <div className="bg-white p-8 rounded-2xl border-2 border-slate-300 shadow-lg space-y-6 max-w-4xl mx-auto">
          ...
          {/* Digital Security & QR Validation Banner in Grades Preview */}
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="text-xs">
                <span className="font-bold text-slate-900 flex items-center gap-1.5">
                  Verifikasi Keaslian Dokumen e-Raport (QR Code Terintegrasi)
                  <span className="text-[10px] bg-emerald-100 text-emerald-800 font-bold px-1.5 py-0.5 rounded">Sah / Resmi</span>
                </span>
                <p className="text-slate-500 text-[11px] mt-0.5">
                  Setiap PDF yang dicetak memuat QR Code unik dengan link otentikasi server sekolah.
                </p>
              </div>
            </div>
            <button
              onClick={() => setIsValidationModalOpen(true)}
              className="px-3 py-1.5 bg-blue-600 hover:bg-blue-500 text-white rounded-lg text-xs font-bold transition-colors cursor-pointer flex items-center gap-1 shrink-0"
            >
      ...
      {/* Raport Validation & QR Code Modal */}
